	...
	function bvPantheonAdminInitHandler() {
		...
		if ($blogvault->getOption('bvActivateRedirect')) {
			...
			if (defined('BVMIGRATEPLUGIN')) {
				...
				wp_redirect('admin.php?page=bv-pantheon-key-config');
			}
		}
	}
...
if (!function_exists('bvPantheonAdminMenu')) :
	function bvPantheonAdminMenu() {
		add_menu_page('bV Pantheon', 'bV Pantheon', 'manage_options', 'bv-pantheon-key-config', 'bvKeyConf');
		if (defined('BVMIGRATEPLUGIN')) {
			add_submenu_page('bv-pantheon-key-config', 'blogVault', 'Migrate Site', 'manage_options', 'bv-pantheon-migrate', 'bvMigrate');
		}
	}
...
if ( !function_exists('bvSettingsLink') ) :
	function bvSettingsLink($links, $file) {
		if ( $file == plugin_basename( dirname(__FILE__).'/blogvault.php' ) ) {
			$links[] = '<a href="' . admin_url( 'admin.php?page=bv-pantheon-key-config' ) . '">'.__( 'Settings' ).'</a>';
		}
		return $links;
	}
...
<div class="bv_page_wide" style="display:block;background:#fff;padding-right:1%;overflow:hidden; margin-right:2.5%;margin-top:1%;"> <!-- SOWP MAIN -->
	...
	<div style="overflow:hidden;">	<!-- SOP 1 -->
			<div class="bv_inside_column1" style="width:100%;max-width:90%;float:left;padding:1% 2.5% 1% 2.5%;border-right:1px solid #ebebeb;overflow:hidden;"> <!-- MCA -->
...
?>
			<div id="message" class="updated" style="padding: 8px; font-size: 16px; background-color: #dff0d8">
						<a class="button-primary" href="<?php echo admin_url('admin.php?page=bv-pantheon-key-config') ?>">Activate blogVault</a>	
